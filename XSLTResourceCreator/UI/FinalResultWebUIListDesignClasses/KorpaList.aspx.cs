                ...
                namespace WebUI
                {
                public partial class KorpaList : System.Web.UI.Page
                {
                
        protected void Page_Load(object sender, EventArgs e)
        {
            MiddletierManager mm = new MiddletierManager();
            List<Korpa> lt = new List<Korpa>();
            
            foreach (Korpa item in mm.FindAll((new Korpa()),"Korpa",true))
            {
                lt.Add(item);
            }
        ...
        int columnidex = GetColumnIndex(row, Korpa.GetIDPropertyName());
        string tekst = this.GridView1.Rows[a].Cells[columnidex].Text;
        Response.Redirect("KorpaEdit.aspx?objectid=" + tekst);
        }
        }
        else
        {
        Response.Redirect("KorpaEdit.aspx");
        }

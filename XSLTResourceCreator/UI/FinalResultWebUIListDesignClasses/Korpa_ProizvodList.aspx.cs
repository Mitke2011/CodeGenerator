                ...
                namespace WebUI
                {
                public partial class Korpa_ProizvodList : System.Web.UI.Page
                {
                
        protected void Page_Load(object sender, EventArgs e)
        {
            MiddletierManager mm = new MiddletierManager();
            List<Korpa_Proizvod> lt = new List<Korpa_Proizvod>();
            
            foreach (Korpa_Proizvod item in mm.FindAll((new Korpa_Proizvod()),"Korpa_Proizvod",true))
            {
                lt.Add(item);
            }
        ...
        int columnidex = GetColumnIndex(row, Korpa_Proizvod.GetIDPropertyName());
        string tekst = this.GridView1.Rows[a].Cells[columnidex].Text;
        Response.Redirect("Korpa_ProizvodEdit.aspx?objectid=" + tekst);
        }
        }
        else
        {
        Response.Redirect("Korpa_ProizvodEdit.aspx");
        }
